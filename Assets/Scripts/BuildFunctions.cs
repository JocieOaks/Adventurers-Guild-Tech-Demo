...
{
    /// <summary>
    /// Indicates the type of <see cref="SpriteObject"/>s being built, so that <see cref="GameManager"/> now how to handle <see cref="BuildFunctions"/>.
    /// </summary>
    ...
    {
        private static MapAlignment s_alignment;
        private static Vector3Int s_areaEnd;
        private static Vector3Int s_areaStart;
        ...
        /// <value>The <see cref="Scripts.Map.Direction"/> any newly built <see cref="SpriteObject"/>s will face.</value>
        ...
        { 
            ...
            {
                s_direction = value;
                s_alignment = Utility.Utility.DirectionToEdgeAlignment(s_direction);
            } 
        }
        ...
        {
            if (spriteObject is WallSprite { IsDoor: true } wall)
            {
                wall.RemoveDoor();
            }
            else
        ...
        public static void PlaceFloor(Room room, int floorType)
        {
            foreach (RoomNode node in room.Nodes)
            {
                ...
                if (Map.Map.Instance.IsSupported(position, MapAlignment.Center))
                    Map.Map.Instance[position].Floor.SpriteIndex = floorType;
            }
        }
        ...
        private static int AlignedCoordinate(Vector3Int position)
        {
