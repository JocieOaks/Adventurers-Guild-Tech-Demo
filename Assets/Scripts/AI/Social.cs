...
public class Social
{
    ...
    IEnumerable GetNearbyPawns(int radius = 10)
    {
        ...
        {
            Vector3Int position = _pawn.CurrentPosition;

            foreach (Pawn pawn in _pawn.CurrentRoom.Occupants)
            {
                Vector3Int relPosition = position - pawn.CurrentPosition;
                if (pawn != _pawn &&
    ...
    void OnTicked()
    {
        ...
        if (Conversation == null)
        {
            ...
            if (random == 0 && CanSpeak())
            {
                ...
                {
                    if (CanSpeakTo(pawn) && Vector3.Dot(Map.DirToVector(_pawn.Direction), pawn.CurrentPosition - _pawn.CurrentPosition) > 0)
        ...
        switch (type)
        {
            case SpeechType.Greet:
                ...
                else
                {
                    ...
                    if (random == 0 && Conversation == null && pawn.Social.Conversation == null && CanSpeak())
                    {
