...
namespace Assets.Scripts.AI.Social
{
    ...
    /// <summary>
    /// The <see cref="SocialAI"/> class controls a <see cref="AdventurerPawn"/>'s social behavior. It runs parallel and independent of the <see cref="Planner"/>'s 
    ...
    {
        private readonly AdventurerPawn _pawn;
        private bool _greetLockout;
        private readonly ListDictionary _lastTimeSpokenTo = new();
        ...
        /// </summary>
        /// <param name="room">The <see cref="Room"/> the <see cref="AdventurerPawn"/> has entered.</param>
        ...
        private bool CanGreet(AdventurerPawn pawn)
        {
            return CanSpeakTo(pawn) && !_greetLockout && (_lastTimeSpokenTo[pawn] == null || GameManager.Instance.Tick - (int)_lastTimeSpokenTo[pawn] > 150);
        }
        ...
        private bool CanSpeakTo(AdventurerPawn pawn)
        {
        ...
        private IEnumerable GetNearbyPawns(int radius = 10)
        {
        ...
        private void Hear(AdventurerPawn pawn, SpeechType type)
        {
            int random;
            switch (type)
            {
            ...
            _lastTimeSpokenTo[pawn] = GameManager.Instance.Tick;
        }
        ...
        private void OnTicked()
        {
            ...
            if (Conversation == null)
            {
                ...
                if (random == 0 && CanSpeak())
                {
                    ...
                    {
                        if (CanSpeakTo(pawn) && Vector3.Dot(Utility.Utility.DirectionToVector(_pawn.Direction), pawn.WorldPosition - _pawn.WorldPosition) > 0)
        ...
        private IEnumerator Respond(AdventurerPawn pawn, SpeechType type, float delay)
        {
        ...
        private void Speak(AdventurerPawn pawn, SpeechType type)
        {
        ...
        private void StartConversation(AdventurerPawn pawn)
        {
